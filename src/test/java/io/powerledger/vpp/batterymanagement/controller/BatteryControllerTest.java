...
class BatteryControllerTest {
    ...
    void should_return_batteries_in_range_with_summary() {
        ...
        when(batteryService.getBatteryByMinAndMaxPostCode(minPostCode, maxPostCode))
                .thenReturn(List.of(battery1, battery2));

        // when
        ResponseEntity<BatteryResponseDto> response = batteryController.getBatteriesInRange(minPostCode, maxPostCode);
        ...
        assertThat(responseBody.getTotalCapacity()).isEqualTo(1100);
        assertThat(responseBody.getAverageCapacity()).isEqualTo(550.0);

        verify(batteryService, times(1)).getBatteryByMinAndMaxPostCode(minPostCode, maxPostCode);
    }
}
